...
app.post('/send-email/invoice', async (req, res) => {
  try {
    const { email, subject, msg, msg2, msg3, msg4, msg5 } = req.body;
        ...
        wkhtmltopdf(msg3, { output: pdfPath3 }, async (err) => {
          ...
          wkhtmltopdf(msg4, { output: pdfPath4 }, async (err) => {
            ...
            const mailOptions = {
              ...
              to: email,
              subject: subject,
              html: msg5, // You can use any of the message content here depending on which one you want in the email body
              attachments: [
...
app.post('/send-email/invoice/single', async (req, res) => {
  // try {
  ...
  // }
  try {
    const { email, subject, msg, msg2 } = req.body;

    const pdfPath = `invoice_${Date.now()}.pdf`;

    // Generate PDF from HTML content
    wkhtmltopdf(msg, { output: pdfPath }, async (err) => {
      if (err) {
        console.error("Error generating PDF:", err);
        return res.status(500).json({ error: "Failed to generate PDF" });
      }
...
app.post("/Admin/quotation/quotation_memo_create", QuotationModel.quotation_memo_create
);
app.get("/Admin/quotation/quotation_memo_single/:id", QuotationModel.quotation_memo_single
);
app.post("/Admin/quotation/quotation_setup_create", QuotationModel.quotation_setup_create
);
app.get("/Admin/quotation/quotation_setup_single/:id", QuotationModel.quotation_setup_single
);
