...
const SchoolShiftCreate = () => {
	...
	const handleChangess = (value) => {
		if (value) {
			// Format the date into MySQL format
			const mysqlFormattedDate = formatToMySQL(value);
			setFromDates(mysqlFormattedDate);

		}
	};
	const [fromDatess, setFromDatess] = useState('');

	const handleChangesss = (value) => {
		if (value) {
			// Format the date into MySQL format
			const mysqlFormattedDate = formatToMySQL(value);
			setFromDatess(mysqlFormattedDate);

		}
	};

	const [fromDatesss, setFromDatesss] = useState('');

	const handleChangessss = (value) => {
		if (value) {
			// Format the date into MySQL format
			const mysqlFormattedDate = formatToMySQL(value);
			setFromDatesss(mysqlFormattedDate);
	...
	const user_create = (event) => {
		...
		const schoolShift = {
			...
			start_time: fromDate,
			late_time: fromDates,
			end_time: fromDatesss,
			early_end_time: fromDatess,
			created_by,
			branch_id
		};
