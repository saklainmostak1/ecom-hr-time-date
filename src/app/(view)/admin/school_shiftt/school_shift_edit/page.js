...
const SchoolShiftEdit = ({ id }) => {
    ...
    //       setMysqlFormattedDate(timeString);
    //     }
    //   }, [formData.start_time]);
    ...
    // 	// Return in MySQL format: yyyy-MM-dd HH:mm
    ...
    //         ...prevData,
    //        start_time: mysqlFormattedDate // Update the dob field in the state
    //     }));
    //     }
    //     else{
...
// time Start
    ...
    const getCurrentDate = () => {
        ...
        const date = new Date(`2000-01-01T${formData.start_time}`); // Ensures a valid Date object
        return date;
    };
    ...
    useEffect(() => {
        if (formData.start_time) {
            const date = new Date(`2000-01-01T${formData.start_time}`);
    ...
    const handleChange = (value) => {
        if (value) {
            const mysqlFormattedTime = formatToMySQL(value);
            setSelectedDate(value);
            setMysqlFormattedDate(mysqlFormattedTime);
            setFormData((prevData) => ({
                ...prevData,
                start_time: mysqlFormattedTime,
            }));
        } else {
            setSelectedDate(null);
            setMysqlFormattedDate("");
    ...
    const getCurrentDates = () => {
        if (!formData.late_time) return null;
        const date = new Date(`2000-01-01T${formData.late_time}`); // Ensures a valid Date object
        return date;
    };

    const [selectedDates, setSelectedDates] = useState(getCurrentDates());
    const [mysqlFormattedDates, setMysqlFormattedDates] = useState("");

    useEffect(() => {
        if (formData.late_time) {
            const date = new Date(`2000-01-01T${formData.late_time}`);
            setSelectedDates(date);
            setMysqlFormattedDates(formatToMySQL(date));
        }
    }, [formData.late_time]);
    ...
    const handleChanges = (value) => {
        if (value) {
            ...
            setMysqlFormattedDates(mysqlFormattedTime);
            setFormData((prevData) => ({
                ...prevData,
                late_time: mysqlFormattedTime,
            }));
        } else {
            setSelectedDates(null);
            setMysqlFormattedDates("");
        }
    };

    const getCurrentDatess = () => {
        if (!formData.end_time) return null;
        const date = new Date(`2000-01-01T${formData.end_time}`); // Ensures a valid Date object
        return date;
    };

    const [selectedDatess, setSelectedDatess] = useState(getCurrentDatess());
    const [mysqlFormattedDatess, setMysqlFormattedDatess] = useState("");

    useEffect(() => {
        if (formData.end_time) {
            const date = new Date(`2000-01-01T${formData.end_time}`);
            setSelectedDatess(date);
            setMysqlFormattedDatess(formatToMySQL(date));
        }
    }, [formData.end_time]);

  

    const handleChangess = (value) => {
        if (value) {
            const mysqlFormattedTime = formatToMySQL(value);
            setSelectedDatess(value);
            setMysqlFormattedDatess(mysqlFormattedTime);
            setFormData((prevData) => ({
                ...prevData,
                end_time: mysqlFormattedTime,
            }));
        } else {
            setSelectedDatess(null);
            setMysqlFormattedDatess("");
        }
    };



    const getCurrentDatesss = () => {
        if (!formData.early_end_time) return null;
        const date = new Date(`2000-01-01T${formData.early_end_time}`); // Ensures a valid Date object
        return date;
    };

    const [selectedDatesss, setSelectedDatesss] = useState(getCurrentDatesss());
    const [mysqlFormattedDatesss, setMysqlFormattedDatesss] = useState("");

    useEffect(() => {
        if (formData.early_end_time) {
            const date = new Date(`2000-01-01T${formData.early_end_time}`);
            setSelectedDatesss(date);
            setMysqlFormattedDatesss(formatToMySQL(date));
        }
    }, [formData.early_end_time]);

  

    const handleChangesss = (value) => {
        if (value) {
            const mysqlFormattedTime = formatToMySQL(value);
            setSelectedDatesss(value);
            setMysqlFormattedDatesss(mysqlFormattedTime);
            setFormData((prevData) => ({
                ...prevData,
                early_end_time: mysqlFormattedTime,
            }));
        } else {
            setSelectedDatesss(null);
            setMysqlFormattedDatesss("");
        }
    };
