...
const AttendanceCreates = () => {
    ...
    const handleChangess = (value) => {
        if (value) {
            ...
            const mysqlFormattedDate = formatToMySQLs(value);
            setFromDate(mysqlFormattedDate);
        }
    };


    const formatToMySQLs = (date) => {
        const year = date.getFullYear();
    ...
    const data = fromDate;
    // const data = fromDate ? formatDates(fromDate) || 0;
    // const datas = startDatetime ? formatDates(startDatetime.datetime) : '';
    // console.log(startDatetime.datetime.slice(0,10))
    // console.log(fromDate ? formatDates(fromDate) : 'no data')

    const foundHoliday = yearlyHolidays.filter(yearlyHoliday =>
        yearlyHoliday.start_date.slice(0, 10) === data.slice(0,10)
    );
    // const foundHoliday = yearlyHolidays.filter(yearlyHoliday =>
    //     yearlyHoliday.start_date.slice(0, 10) === data
    // );
    ...
    const handleChangesss = (value) => {
        if (value) {
            ...
            setAssetInfo((prevAssetInfo) => ({
                ...prevAssetInfo,
                quotation_date: mysqlFormattedDate
            }));
        }
    };
