...
const QuotationLists = ({ searchParams }) => {
    ...
    const handleChangess = (value) => {
        if (value) {
            // Format the date into MySQL format
            const mysqlFormattedDate = formatToMySQL(value);
            setFromDate(mysqlFormattedDate);
        }
        else{
            setFromDate('')
        }
    ...
    const filteredBtnIconEmailSms = brandList.filter(btn =>
        btn.method_sort === 11
    );
    const filteredBtnIconInvoicePrint = brandList.filter(btn =>
        btn.method_sort === 12
    );
    const filteredBtnIconMemo = brandList.filter(btn =>
        btn.method_sort === 14
    );
    ...
    const sale_print_single = async (id) => {
        try {
            ...
            const html = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/quotation/quotation_list_print_single`, {
                ...
                body: JSON.stringify({
                    searchResults, orientation, fontSize, selectedPrintSize, extraColumnValue, quotation_current_date_count, designationemployee
                }),
            });
    ...
    const sale_pdf_download_single = async (id) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/quotation/quotation_list_pdf_single`, {
                ...
                body: JSON.stringify({
                    designationemployee, searchResults, orientation, fontSize, selectedPrintSize, quotation_current_date_count
                    // Other parameters if needed
                }),
                                            ...
                                            <div class="form-group row student">
                                                ...
                                                <div class="col-md-10">
                                                    <div class="input-group ">
                                                        ...
                                                        <select name="zoom_size" class="form-control form-control-sm  trim integer_no_zero zoom_size d-none" id="zoom_size">
                        ...
                        <div className="body-content bg-light">
                            <div className="border-primary shadow-sm border-0">
                                <div className="card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                                    ...
                                    {/* <div className="card-title font-weight-bold mb-0 card-header-color float-right">
                                        <Link className='mr-2' href={`/Admin/quotation/quotation_memo?page_group`} >
                                            {filteredBtnIconMemo?.map((filteredBtnIconEdit => (
                                                <button
                                                    key={filteredBtnIconEdit.id}
                                                    className="btn btn-sm btn-info"

                                                >
                                                    {filteredBtnIconEdit.display_name}
                                                </button>
                                            )))}
                                        </Link>
                                    </div> */}
                                                                        ...
                                                                        <tr key={loan.id}>
                                                                            ...
                                                                            <td>
                                                                                ...
                                                                                <div className="flex items-center ">
                                                                                    ...
                                                                                    {filteredBtnIconInvoice.map((pdf => (
                                                                                        <div className="dropdownss" style={{ position: "relative", display: "inline-block" }} key={loan.id}>
                                                                                            ...
                                                                                            >
                                                                                                ...
                                                                                                ></a> Quotation <i class="fas fa-angle-down"></i>
                                                                                            </button>
                                                                                    ...
                                                                                    <Link href={`/Admin/quotation/quotation_email_sms/${loan.id}?page_group=${page_group}`}>
                                                                                        {filteredBtnIconEmailSms?.map((filteredBtnIconEdit => (
                                                                                            <button
                                                                                                key={filteredBtnIconEdit.id}
                                                                                                title='Edit'

                                                                                                className='btn btn-sm btn-dark mt-2 ml-2'
                                                                                            >
                                                                                                {filteredBtnIconEdit.display_name}
                                                                                            </button>
                                                                                        )))}
